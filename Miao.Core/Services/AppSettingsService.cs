...
namespace Miao.Core.Services
{
    public class AppSettings
    {
        ...
        // TranslationEngine: "DichNgay" or "CT2"
        public string TranslationEngine { get; set; } = "DichNgay";
        public string DichNgayEndpoint { get; set; } = "https://dichngay.com/translate/text";
        public string CTranslate2Endpoint { get; set; } = "http://127.0.0.1:5001/translate";
    }
    ...
    public class AppSettingsService
    {
        ...
        public static AppSettingsService Instance =>
            _instance ?? throw new InvalidOperationException(
                "AppSettingsService chưa được khởi tạo. Gọi AppSettingsService.Initialize(baseFolder) " +
                "ở entry point của Miao.Desktop hoặc Miao.Android trước khi dùng.");

        // Gọi 1 lần duy nhất lúc app khởi động, từ project platform tương ứng
        // (Miao.Windows truyền ApplicationData, Miao.Android truyền Context.FilesDir)
        ...
        private AppSettingsService(string baseFolder)
        {
            ...
            {
                Settings = JsonSerializer.Deserialize<AppSettings>(File.ReadAllText(_configPath)) ?? new AppSettings();
            }
            else
            {
                Settings = new AppSettings { DataFolder = configFolder };
            }
            ...
            if (string.IsNullOrWhiteSpace(Settings.DichNgayEndpoint))
                Settings.DichNgayEndpoint = "https://dichngay.com/translate/text";

            if (string.IsNullOrWhiteSpace(Settings.CTranslate2Endpoint))
                Settings.CTranslate2Endpoint = "http://127.0.0.1:5001/translate";
