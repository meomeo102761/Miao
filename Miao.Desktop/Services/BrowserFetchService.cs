...
namespace Miao.Desktop.Services
{
    ...
    /// FetchFanqieChapterAsync của bản gốc đã bị xóa vì không nơi nào gọi tới —
    /// FanqieDownloadSource lấy nội dung chương qua FanqieDecryptService (gọi API trực tiếp
    /// bằng HttpClient), không qua WebView2.
    /// </summary>
    public class BrowserFetchService : IPageFetcher, IScreenshotFetcher
    {
        ...
        private async Task EnsureInitializedAsync()
        {
            ...
            {
                Width = 1, Height = 1, WindowStyle = System.Windows.WindowStyle.None,
                ShowInTaskbar = false, Left = -2000, Top = -2000
            };
        ...
        public async Task<string> FetchHtmlFastAsync(string url, int waitMs = 500)
        {
            await EnsureInitializedAsync();
            ...
            try
            {
                ...
                if (completedTask != tcs.Task)
                    throw new TimeoutException($"Không thể tải trang trong 30 giây: {url}");

                if (!await tcs.Task)
                    throw new Exception($"WebView2 không tải được trang: {url}");
            }
            finally
            {
                _webView.NavigationCompleted -= Handler;
        ...
        public async Task<byte[]> CaptureScreenshotAsync(string url, int extraWaitMs = 2500)
        {
            await EnsureInitializedAsync();
